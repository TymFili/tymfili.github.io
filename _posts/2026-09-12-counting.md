...
1. **Well-defined codomain ($$h(A) \subseteq B$$):**  
   If $$x \in C$$, $$h(x) = f(x) \in B$$. If $$x \notin C$$, $$x \notin A_0 = A \setminus B$$, so $$x \in B$$.
2. **Injectivity:** Let $$h(x) = h(y)$$.  
   * If $$x, y \in C$$, $$f(x) = f(y) \implies x = y$$.  
   * If $$x, y \notin C$$, $$x = y$$.  
...
Thus $$h \colon A \longrightarrow B$$ is a bijection.
...
$$
is an injection from $$A$$ into its subset $$g(B)$$. Applying **Case 1** to $$(A, g(B))$$ with injection $$g \circ f$$ yields a bijection $$h \colon A \longrightarrow g(B)$$.

Composing with $$g^{-1}$$ yields the bijection:
$$
