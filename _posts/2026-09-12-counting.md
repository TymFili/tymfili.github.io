...
Reflexivity ($$A \le_c A$$) and transitivity ($$A \le_c B \land B \le_c C \rightarrow A \le_c C$$) are trivial by composing identity and injective functions. 

However, **antisymmetry** is deceptively difficult:
$$
A \le_c B \land B \le_c A \implies A \sim B
$$
If you have an injection from $$A$$ into $$B$$ and an injection from $$B$$ into $$A$$, can you glue them together to construct a genuine bijection?
...
Once we know that mutual injections establish equinumerosity, can an infinite set ever be as large as its collection of subsets?
...
Because the singleton map $$x \mapsto \{x\}$$ is an injection $$A \hookrightarrow \mathcal{P}(A)$$, Cantor’s theorem proves that the power set is strictly larger:
$$
A <_c \mathcal{P}(A)
$$
